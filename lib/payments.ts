...
export async function handleCheckoutSessionCompleted({
  ...
  console.log("Checkout session is completed", session);
...
async function createOrUpdateUser({
  ...
  try {
    ...
    if (user.length === 0) {
      await sql`INSERT INTO users (email, full_name, customer_id, price_id, status ) VALUES (${email},${fullName},
           ${customerId}, ${priceId}, ${status})`;
    }
  } catch (error) {
